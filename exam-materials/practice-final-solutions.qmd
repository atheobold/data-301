...
It tells me that almost all the Pokemon were properly classified in the training
data. Only four of them from the second type (Grass) were misclassified as
either Fire or Electric.
...
`k` = 3 had the highest F1 score, but this value is fairly close to `k` = 1. 
Based on the table, as `k` increases the precision increases and recall
decreases. So, I might be more inclined to use a `k` = 1, where **both** 
...
I would recommend the Logistic Regression, because the cross-validated F1 scores
were less than 1% away from KNN, and Logistic Regression provides more
