---
title: "Practice Exam 2 - Solutions by Dr. Theobold"
subtitle: "DATA 301"
editor: source
---
...
No - standardizing is not needed in a decision tree, since we don't use distances to fit our model. It would only make the tree harder to interpret.

**Question 2: Should I chosen `remainder = drop` in my preprocesser? Why or why not?**

No - then you would be left with only the "Legendary" column as a predictor. 
This would remove `HP`, `Attack`, `Defense`, and `Speed` as predictors. 
...
Not necessarily. This accuracy is the *training accuracy*. Just because the tree was able to "sort" the Pokemon in the training set doesn't mean it would do well on other datasets. We would want to cross-validate to get a better estimate of
the testing accuracy.

**Question 4: What does the confusion matrix tell me about this model?**

The rows of the confusion matrix are the actual observations in the `Electric`,
`Fire`, `Grass`, and `Water` groups (alphabetically ordered). The columns are
the predicted values of these groups. The diagonal represents starter types that
were correctly classified (were `Electric` and were predicted to be `Electric`). 
The off diagonal values are where the model had misclassifications. 

There are only two places were there misclassifications. Two observations that
were `Grass` were misclassified as `Fire` and two observations that were `Water`
were misclassified as `Fire`. 
...
It was very important to scale the variables! The KNN model uses distances to
find nearest neighbors. If we don't scale, then those distance measurements will
be dominated by the variable with the largest numbers.
...
In the context of this problem, it seems like you would want to find as many of the legendary Pokemon as possible. Meaning, you would want to decrease your false negative rate. False negatives are associated with recall, so I would choose the `k` with the largest recall. Based on the table, this would be a `k` of 1. 

::: {.callout-tip}
This not the only argument that could be made! Any argument needs to have these elements: 

- What metric(s) you are prioritizing 
- Why
- What value of k that leads you to choose
:::

**Question 7: It looks like both precision and recall are zero at k = 100. Does this make sense, or did I do something wrong?**

<!-- 
precision: false positives - predicting legendary when it is non-legendary
recall: false negatives - predicting non-legendary when it is legendary
-->

This makes sense. Since the data is over 90% Non-Legendary (65 / 800), when 
`k` = 100, we will essentially always have a majority of Non-Legendary
neighbors. The majority of the neighbors being Non-Legendary means we will
predict Non-Legendary for every Pokemon, so the precision for the Legendary
Pokemon is 0 and the recall is 0.
...
-   For each point of HP the Pokemon has, the probability of it being Legendary
increases by 0.04.

-   For each point of attack the Pokemon has, the probability of it being
Legendary increases by 0.014

-   For each point of defense the Pokemon has, the probability of it being
Legendary increases by 0.035

-   For each point of speed the Pokemon has, the probability of it being
Legendary increases by 0.056
...
Almost! But it's not the *probability* that increases, it's the *log-odds*.
...
Since I decided to prioritize recall, I would choose the KNN model. The recall
for the KNN model is almost 10% higher than the logistic regression, which is 
a pretty substantial difference. 

::: {.callout-caution}
Your decision here should be consistent with what you used to decide what value
of `k` was best in Question 6.
:::

::: {.callout-tip}
If the metrics of two models are close (e.g., an F1 of 65% and 67%), then you 
should also consider the complexity of the model. How easy would it be to 
explain a KNN model to someone? What about a logistic regression? That should 
influence the model you choose!
:::
