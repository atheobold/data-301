...
::: {.midi}
In the last class a student (smartly) asked what would happen if there 
were ties. 

- For example, if 2 of the 5 neighbors were from Group A, 2 were from 
Group B, and 1 was from Group C. 
- The probabilities for Group A and Group B would be equal.
:::

. . .

::: {.midi}
By default, `KNeighborsClassifier()` uses `weights = "uniform"`, which selects
the **first** occurrence of the maximum value. Meaning, the group that comes
first in `.classes_` is selected. 
:::

. . .

::: {.midi}
You can change this to prioritizing the group with the smallest distances by 
using `weights = "distance"`. 
:::
<!-- https://chatgpt.com/share/e/699532be-444c-8005-a692-8a0ce5e23242 -->
...
**You should NOT use ordinary regression for a classification problem!**
...
The `"l1" penalty does lasso regression. The `elasticnet` penalty does lasso +
ridge regression.
:::
...
```{python}
#| code-line-numbers: false
#| label: 70-percent-threshold-precision
...
```{python}
#| code-line-numbers: false
#| label: 70-percent-threshold-recall
...
-   To fit a **regression model** (i.e., coefficients times predictors) to a
**categorical response**, we use **logistic regression**.

-   Coefficients are interpreted as "One unit increase in predictor is
associated with a \[something\] increase in the **log-odds** of Category 1."

-   We still use **cross-validated metrics** to decide between KNN and Logistic
regression, and between different feature sets.

-   We still report **confusion matrices** and sometimes
**precision-recall curves** of our final model.
