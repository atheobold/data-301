...
- [Dataset on coffee attributes](https://github.com/atheobold/data-301/blob/main/exam-materials/coffee_clean.csv)
  ...
  * Raw data can be downloaded [here (link to data)](https://github.com/atheobold/data-301/blob/main/exam-materials/coffee_kaggle.csv)
...
::: {.midi}
-   **OneHotEncode:** Create dummy variables (columns of 0s and 1s) for every
level of a categorical variables.
...
-   In **scikit-learn**, use the `pairwise_distances()` function to get back a
*2D numpy array* of distances.

. . .

[But what if our data are text instead of categories?]{.large}
...
```{python}
#| code-line-numbers: false
#| label: bag-of-words-for-one-file-series
...
::: {.small}
::: {.incremental}
- `Counter(doc.split())` splits each document by word and counts word frequencies
- `for doc in docs.values()` iterates over every document
- `pd.Series()` converts the `Counter()` object into a Pandas Series
:::
:::
...
<!-- The default values for `token_pattern` is `r"(?u)\b\w\w+\b"` which keeps only -->
<!-- word characters (a–z, A–Z, digits) that are at least 2 characters long and  -->
<!-- ignores punctuation.  -->

<!-- r"[\S]+"` declares a regular expression that treats every sequence of characters that is not whitespace (`[\S]`) as a single token. -->
<!-- ::: -->
<!-- ::: -->
...
bow_totals = bow_dataframe.sum(axis = "columns")
bow_totals
```
...
bow_tf = bow_dataframe.divide(bow_totals, axis = "rows")
bow_tf
```
:::
...
And adjust for the inverse document frequencies:
...
bow_tf_idf = bow_tf.multiply(bow_log_idf, axis = "columns")
...
## Not Adding 1 to the IDF

The `smooth_idf` argument in  `TfidfVectorizer()` controls whether to add 1 
to the IDF or not. 

</br>

If you **do not** want to add 1 to the IDF, then you can specify 
`smooth_idf = False`:
