...
Essentially `unstack()` has pivoted the `sex` column from **long format** 
(where the values are included in one column) to **wide format** where each 
value has its own column. 
:::
...
**What cross-tabulation would you expect if we changed the order of the
variables? In other words, what would happen if `"sex"` came first and 
`"pclass"` came second?**
...
> Of all the passengers on the Titanic, 11% were female passengers riding in
> first class.
...
Here we ***conditioned on*** the passenger being female, and then looked at the
***conditional distribution*** of `pclass`.
...
Here we ***conditioned on*** the passenger being in first class, and then looked
at the ***conditional distribution*** of `sex`.
...
-\> Of all *female passengers*, what is the conditional distribution of
*class*?
...
::: {.midi}
When we study two variables, we call the individual one-variable distributions
