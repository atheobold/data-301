...
import pandas as pd
breast_df = pd.read_csv("data/BreastTissue.csv")
```
...
(
  ...
  labs(x = "I0 (Impedivity at 0 kHz)", 
       y = "Phase Angle at 500 kHz")
    )
```
...
Your grid should include:

- $k$ (the number of neighbors)
- the distance metric (manhattan, euclidean)
- whether the mean should be used when standardizing
- whether the standard deviation should be used when standardizing
...
conf_mat["Total"] = conf_mat.sum(axis = "columns")
...
-   What's the precision for fraudulent transactions (`1`)?

-   What's the recall for fraudulent transactions (`1`)?
:::
