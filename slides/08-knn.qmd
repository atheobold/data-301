...
In addition to accomplishing the given task, your grade on each problem will
also depend on:

- whether your solution uses tools taught in this class, and
- the efficiency of your solution (e.g., did you use a `for` loop when it was
not necessary?)
...
[**Login to Canvas.**]{.large}

</br>

[**Login to your Google account.**]{.large}
...
auctions. 
    + The price is relative to the price of the 1961 vintage. 
...
(
  ...
  theme_bw() +
  labs(x = "Average Summer Temperature (Celsius)", 
       y = "Winter Rainfall (mm)")
)
```
:::
...
::: {.fragment}
::: {.callout-tip}
Why don’t we compute the mean and standard deviation using all the data
(training + test)?
:::
:::
:::
...
<!-- Using the test data to compute the mean and standard deviation, -->
<!-- allows information from the test set to influence training. -->
...
::: {.fragment}
::: {.callout-important}
How would you interpret this price?
:::
:::
:::
:::
...
::: {.callout-tip}
# I'm using the `preproc` I made earlier! 
...
We're borrowing from statistical notation here where `y` is the "response" or 
"target" variable and `X` is the matrix of "predictors" or "features". 
:::
