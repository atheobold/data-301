...
What are the indices of the output of `.groupby()`?

::: {.midi}
```{python}
#| code-line-numbers: false
#| label: group-by-median-indices
...
```{python}
#| code-line-numbers: false
#| label: group-by-median-reset-indices
...
What if I wanted to know if different airlines had different delays?
...
(
  df[small_carriers]
  .groupby(by = ["origin", "carrier"])["dep_delay"]
  .median()
)
```
:::
...
What are the indices of this output?

## Reseting Multiple Indices

::: {.small}
```{python}
#| code-line-numbers: false
#| label: group-by-multiple-reset-index
#| output-location: column
